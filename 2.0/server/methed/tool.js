...
let cont = (cont) => {
    ...
    if (cont[contL - 1] === 13 && cont[0] === 1) return {cardType: 3, L: contL, value: 1};
...
//判断牌型
let compare = (current) => {
    console.log("compare:"+JSON.stringify(current))
...
// console.log(contrast({cardType: 1, L: 1, value: 11}, {cardType: 1, L: 1, value: 12}));
 ...
 * @param {黑五} five 
 * @param {所有牌的牌组} cards 
 * @param {已出完牌的用户索引} ends 
 */
function victoryOrDefeat(banker, five, cards, ends) {
    if (ends.length === 1 && banker === five && !cards[five].length) return {state: 1, win: [banker], ends};
    if (banker !== five) {
        if (ends.length === 2) {
